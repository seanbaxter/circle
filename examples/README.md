...
[**hello.cxx**](hello/hello.cxx) [(output)](hello/output.txt)  
```cpp
...
[**scopes.cxx**](scopes/scopes.cxx) [(output)](scopes/output.txt) 
```cpp
...
[**fibonacci.cxx**](fibonacci/fibonacci.cxx) [(output)](fibonacci/output.txt)  
```cpp
...
[**serialize.cxx**](serialize/serialize.cxx) [(output)](serialize/output.txt)
```cpp
template<typename type_t>
...
[**serialize2.cxx**](serialize/serialize2.cxx) [(output)](serialize/output2.txt)  
```cpp
template<typename type_t>
...
[**format.cxx**](format/format.cxx) [(output)](format/output.txt)  
```cpp
...
[**eprintf.cxx**](eprintf/eprintf.cxx)
```cpp
int main() {
  int x = 101;
  double y = 3.14;
...
[**special.json**](special/special.json)  
```json
{
  "sin" : {
    "f"          : "sin(x)",
...
[**special.cxx**](special/special.cxx)  
```cpp
...
We'll take the [**special.cxx**](special/special.cxx) file and break it subtly. C++ allows floating-point literals in the form -.0083333, but JSON does not: it requires an integer 0 between the _-_ and the _._, like so: -0.0083333. We can't expect an error from the C++ tokenizer, because C++ doesn't tokenize this text--that's done by the JSON parser.
...
[**kernel1.cxx**](kernel/kernel1.cxx) [(output)](kernel/output1.txt)
```cpp
// Load a JSON file.
...
[**kernel2.cxx**](kernel/kernel2.cxx) [(output)](kernel/output2.txt)
```cpp
...
[**output2.txt**](kernel/output2.txt)
```
...
[**sfinae.cxx**](sfinae/sfinae.cxx) [(output)](sfinae/output.txt)  
```cpp
template<typename type_t>
void go(type_t& obj) {
  // Try to set obj.x = 1.
...
[**dispatch.cxx**](dispatch/dispatch.cxx) [(output)](dispatch/output.txt)
```cpp
template<
  size_t I,
